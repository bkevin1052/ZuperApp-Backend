...
loginGoogle = router.post('/loginGoogle', async (req, res) => {
    ...
    if(user){
        res.json({token: '', username: user.username,mensaje: "Usuario ya registrado, favor ingrese con el correo y password!",codigo: '100'});
    }else{
        const newUser = new User({ username, password, email,name,surname});
        console.log(newUser)
        await newUser.save()
        ConfigCorreoGoogle(email, aleatorio);
        res.json({token: '', username: newUser.username,mensaje: "Usuario registrado correctamente, hemos enviado su password al correo registrado!",codigo: '100'});
...
updateProfilePassword = router.post('/updateProfilePassword', async (req, res) => {
    ...
    var oldPass = crypto.createHash('md5').update(req.body.inputPassword).digest("hex");

    if(pass == oldPass){
        if(user){

            let doc = await User.findOneAndUpdate({username:user.username}, {password:newPass});
        ...
        res.json({token: '', username: user.username,mensaje: "Contraseña antigua no coincide.",codigo: '406'});
    }
    res.end();
